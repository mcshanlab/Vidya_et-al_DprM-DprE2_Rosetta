...
position_mapping = {
    "B1": "M1", "B2": "N2", "B3": "N3", "B4": "W4", "B5": "T5", "B6": "E6",
    "B7": "E7", "B8": "E8", "B9": "V9", "B10": "Q10", "B72": "S72", "B73": "G73", "B74": "E74", "B75": "L75", "B76": "G76", "B77": "E77", "B78": "E78", "B79": "D79", "B80": "L80", "B81": "N81", "B82": "R82", "B83": "V83", "B84": "A84", "B85": "G85", "B86": "G86"
}
...
def create_heatmap_from_sc_files():
    ...
    # Sort positions numerically by the numeric value in the position string
    sorted_positions = sorted(positions_set, key=lambda x: int(''.join(filter(str.isdigit, x))))
    ...
    ax = sns.heatmap(heatmap_matrix, annot=True, fmt='.1f', xticklabels=updated_positions, yticklabels=sorted_mutants,
                     cmap=cmap, norm=norm, cbar=True, linewidths=0.25, linecolor='white', annot_kws={'size': 8})  # Add outline with white color

    # Set horizontal y-axis labels
    plt.yticks(rotation=0)  # This makes the y-axis labels horizontal
    plt.xlabel("DprE2 Wild-Type Residues")
    plt.ylabel("Mutant Amino Acids")

    # Modify the colorbar label and rotate it by 270 degrees
    cbar = ax.collections[0].colorbar
    cbar.set_label("ΔG (kcal/mol)", fontsize=12, rotation=270)
